...
int main(int argc, char** argv)
{
	...
	if(argc == 2) {
		...
		fprintf(stderr, "No log file specified, writing to stdout\n");
		useFile = 0;
	}
	...
	fprintf(stderr, "Waiting for message from kernel\n");
	...
	while (1)
	{
		...
		l2 = htons(l);
		fwrite(&l2, 1, sizeof(unsigned short), useFile ? out : stdout);
		ret = fwrite(cmsg->data, 1, l, useFile ? out : stdout);
		if (count % 100 == 0)
